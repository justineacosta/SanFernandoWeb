...
/**
 * Create a new news category. Unlike services (which auto-suffix a
 * colliding slug with -2, -3…), categories reject on collision: the id is a
 * stable foreign key for news_articles.category_id, and silently
...
/** Rename a category's label. The id (and existing articles' category_id) never changes. */
export async function renameNewsCategory(
  id: string,
  values: NewsCategoryValues,
): Promise<ActionResult> {
  const actor = await requireSuperAdmin();
 ...
 * news_articles.category_id references these rows, and a hard delete
 * would orphan a published article.
 */
