...
- **Rejection copy is always `"Incorrect email or password."`** — for a bad password, an over-limit attempt, and a failed challenge alike. A distinct message per case tells an attacker which check they tripped. Two pre-existing exceptions are deliberate and must NOT be changed to match: the Zod failure returns `"Enter your email and password."` (a client-side validation prompt, reached before any account is looked up), and the disabled-account path returns `"This account is disabled. Contact the barangay administrator."` (reached only *after* correct credentials, so it reveals nothing an attacker did not already supply).
