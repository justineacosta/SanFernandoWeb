...
- **Rejection copy is always `"Incorrect email or password."`** — for a bad password and an over-limit attempt alike. A distinct message per case tells an attacker which check they tripped. Three exceptions are deliberate and must NOT be changed to match: the Zod failure returns `"Enter your email and password."` (a client-side validation prompt, reached before any account is looked up); the disabled-account path returns `"This account is disabled. Contact the barangay administrator."` (reached only *after* correct credentials, so it reveals nothing an attacker did not already supply); and — **amended 2026-08-03 during Task 5** — the failed-challenge path returns `TURNSTILE_FAILURE_MESSAGE`. That last one originally shared the generic copy, and it was wrong: the page can only server-render the challenge from the IP key, never from `login:email:*` (no email is known at render time), so a staff member whose own address is flagged gets no widget, submits the correct password with no token, and would be told their working credential was incorrect — sending them to reset a password that is fine. The distinct copy leaks nothing the UI does not already show, since the widget appears on the very next render regardless. It still never distinguishes a real account from an unknown one, nor a right password from a wrong one.
