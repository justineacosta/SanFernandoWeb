...
| `0037` (bucket MIME allow-lists, status-aware pairs) | **Production enforces `0037` today but does NOT yet run this branch's `copyObjects` content-type fix.** Until this branch deploys, a publish or archive whose downloaded blob resolves no content type fails closed. Deploy this branch before the next admin publish. The fix itself is order-safe (constrains uploads only) — it's hardening backlog Task 7's `copyObjects` content-type fix (`src/lib/media-lifecycle.ts`); without it, `promoteMedia` re-uploads with a possibly-undefined content type that the allow-list rejects. `0037` was applied to staging then production 2026-08-12. On **staging**, verified with a real publish/archive round trip on an image kind and the PDF kind, allow-list enforcing — both succeeded. On **production**, verified read-only (`storage.buckets` values match exactly); no round trip was run there — staging's already proved the mechanism, and this branch isn't deployed to production yet, so a live-database write carried real risk for no new information (`.claude/storage.md`) |
